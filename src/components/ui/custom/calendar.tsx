...
function genYears(yearRange = 50, yearFrom: "after" | "before" | "both") {
...
function getRemainingMonthsInCurrentYear() {
  const now = new Date();
...
function Calendar({
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const YEARS = React.useMemo(() => genYears(yearRange, yearFrom), []);
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        month: "flex flex-col items-center  space-y-4 w-full",
          ...
          return (
            <div className="inline-flex w-full gap-2">
              <Select
                ...
                <SelectTrigger className="w-3/5 gap-1   focus:bg-accent focus:text-accent-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {MONTHS.map((month) => (
                    <SelectItem
                      ...
                      disabled={
                        ...
                        new Date().getFullYear().toString()
                          ? yearFrom === "before"
                            ? getRemainingMonthsInCurrentYear()
                                .slice(0, -1)
                                .includes(month.label)
                            : !getRemainingMonthsInCurrentYear().includes(
                                month.label
                              )
                          : false
                      }>
              ...
              <Select
                ...
                <SelectTrigger className="w-2/5 gap-1   focus:bg-accent focus:text-accent-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {YEARS.map((year) => (
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild disabled={popoverDisabled}>
          <Button
            ...
            ref={buttonRef}>
            <Image src={CalendarIcon} alt="calendar" width={20} height={20} className="mr-2 h-4 w-4" />
            {displayDate ? (
              format(
                displayDate,
                granularity === "day"
